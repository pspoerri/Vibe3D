...
/**
 * The recovery lane, and the reason `lastSource` is a record of its own.
 *
 ...
 *
 * `unknown` on purpose for the session: what comes back was written by some
 * older version of this app, or by a store a browser corrupted, so only the
 * caller's parser may say what it is. Never rejects, and the `try` wraps the
 * `await` rather than being a `.catch()` — idb-keyval opens the database inside
 * this call, so a browser with site data blocked throws synchronously, during
 * App boot, where e2e/smoke.spec.ts asserts zero page errors.
 */
