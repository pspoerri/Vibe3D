 ...
 */
export function nameFromFirstPrompt(session: Session, prompt: string, now: number): Session {
  const doc = currentDoc(session)
...
/** The root this document descends from, following parentId up. */
function rootOf(session: Session, doc: Doc): string {
  const byId = new Map(session.docs.map((d) => [d.id, d]))
  const seen = new Set<string>()
  let node = doc
  // `seen` guards a parentId cycle, which a corrupt store can hand us.
  while (node.parentId !== null && !seen.has(node.id)) {
    seen.add(node.id)
...
/**
 * Every document descending from the same root, oldest first. Ordered by
 * creation rather than by tree depth so two forks of the same parent get
 * distinct numbers instead of two rows both labelled v2.
 */
export function versionFamily(session: Session, id: string): Doc[] {
  const doc = session.docs.find((d) => d.id === id)
  if (!doc) return []
  const root = rootOf(session, doc)
  return session.docs
    .filter((d) => rootOf(session, d) === root)
...
export function versionNumber(session: Session, id: string): number {
  const family = versionFamily(session, id)
  if (family.length < 2) return 0
  return family.findIndex((d) => d.id === id) + 1
}
...
function reviveDoc(raw: unknown, now: number): Doc | null {
  ...
  return {
    id: d.id,
    name: typeof d.name === 'string' && d.name !== '' ? d.name : suggestName(d.source, []),
    source: d.source,
...
export function reviveSession(
  ...
  const docs = rows.map((row) => reviveDoc(row, now)).filter((doc) => doc !== null)
  const first = docs[0]
 ...
 */
const LEAD_IN =
  /^(?:please\s+)?(?:could you\s+|can you\s+)?(?:(?:make|create|build|design|draw|model|generate|give)\s+(?:me\s+)?)?(?:an?|the)\b\s*/i
